		...
		if (packet.stream_index == videoStream)
		{
			...
			if (gotFrame)
			{
				if (pFrame->key_frame == 1)//这里取到关键帧数据   
				{
					...
					_snprintf(szImgFName, sizeof(szImgFName), "%s_%08d.jpg", img_outfile, i);
...
static int save_frame_jpeg(const AVFrame *pFrameYUV, const char *pImgFName, int width, int height)
{
	...
	if (avio_open2(&pPicFmtCtx->pb, pImgFName, AVIO_FLAG_READ_WRITE,
		NULL, NULL) < 0)
	{
		printf("error!\n");
		return -1;
	...
	if (avcodec_open2(pPicCodecCtx, pPicCodec, NULL) < 0)
	{
		printf("error!\n");
		return -1;
	}
	...
	if (avcodec_encode_video2(pPicCodecCtx, &picPkt, pFrameYUV, &nGotPic) < 0)
	{
		printf("error!\n");
		return -1;
	}
